...
const OutlineEdit: React.FC<OutlineEditProps> = ({
  ...
  const [progressMessage, setProgressMessage] = useState(''); // 新增：进度信息
  const [editingItem, setEditingItem] = useState<string | null>(null);
  const [editTitle, setEditTitle] = useState('');
  const [editDescription, setEditDescription] = useState('');
  const [editWordCount, setEditWordCount] = useState<number | undefined>(undefined);
  ...
  const handleExpandUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setMessage({ type: 'success', text: `参考目录上传成功：${file.name}` });
      } else {
  ...
  const handleGenerateOutline = async () => {
    ...
    try {
      ...
      setMessage(null);
      setStreamingContent('');
      setProgressMessage('开始生成目录...'); // 初始化进度信息
      ...
      while (true) {
        ...
        for (const data of dataLines) {
          ...
          try {
            ...
            // 处理错误
            if (parsed.error) {
              throw new Error(parsed.message || '目录生成失败');
            }
          } catch (e) {
            // 如果不是JSON格式错误，则抛出
  ...
  const startEditing = (item: OutlineItem) => {
    ...
    setEditWordCount(item.target_word_count);
  };
      ...
      return items.map(item => {
        if (item.id === editingItem) {
          return {
            ...
            target_word_count: editWordCount
          };
        }
        if (item.children) {
          return {
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <input
                    type="number"
                    value={editWordCount || ''}
                    onChange={(e) => setEditWordCount(e.target.value ? parseInt(e.target.value) : undefined)}
          ...
          <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-md">
            <div className="flex items-center">
              ...
              <span className="text-sm text-green-800">
                已上传参考目录：<span className="font-medium">{expandFile.name}</span>
              </span>
            </div>
          </div>
        )}
        ...
        {!projectOverview && !techRequirements && (
          <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-md">
            <p className="text-sm text-yellow-800">
              请先在"文档解析"步骤中完成文档分析，用户确认需要生成文档的框架结构。
            </p>
          </div>
        )}
