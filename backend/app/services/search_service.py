...
def _check_seleniumbase():
    global SELENIUMBASE_AVAILABLE
    try:
...
class SearchService:
    ...
    def search(self, query: str, max_results: Optional[int] = None) -> List[Dict[str, str]]:
        ...
        try:
            ...
            logger.warning(f"Bing搜索失败: {e}，回退到百度")

        # 策略2: 百度
        try:
            return self._search_baidu(query, limit)
        except Exception as e:
            logger.error(f"百度搜索也失败: {e}")
            raise Exception(f"搜索服务异常: Bing和百度均不可用")
    ...
    def _search_bing(self, query: str, max_results: int = 5) -> List[Dict[str, str]]:
        ...
        for item in items[:max_results]:
            ...
            href = link.get('href', '')
            # 摘要
            body_el = item.select_one('.b_caption p') or item.select_one('p')
            body = body_el.get_text(strip=True) if body_el else ''
            if title and href:
                search_results.append({"title": title, "href": href, "body": body})
        
        logger.info(f"Bing搜索完成，查询: {query}, 结果数量: {len(search_results)}")
        return search_results

    def _search_baidu(self, query: str, max_results: int = 5) -> List[Dict[str, str]]:
        """使用百度网页搜索作为备选"""
        headers = {
            'User-Agent': self._get_random_user_agent(),
            'Accept-Language': 'zh-CN,zh;q=0.9,en;q=0.8',
        ...
        soup = BeautifulSoup(resp.text, 'html.parser')
        items = soup.select('.result, .result-op')
        ...
        for item in items[:max_results]:
            ...
            # 摘要
            body_el = item.select_one('.c-abstract') or item.select_one('.content-right_8Zs40')
            body = body_el.get_text(strip=True) if body_el else ''
            if title and real_url:
                search_results.append({"title": title, "href": real_url, "body": body})

        logger.info(f"百度搜索完成，查询: {query}, 结果数量: {len(search_results)}")
        return search_results
    ...
    def load_url_content(self, url: str, max_chars: int = 5000) -> Dict[str, str]:
        ...
        if _check_playwright():
            try:
                # 在新线程中运行异步 Playwright，避免嵌套事件循环问题
                def _run_playwright():
    ...
    async def load_url_content_async(self, url: str, max_chars: int = 5000) -> Dict[str, str]:
        """
        异步读取链接内容，使用多种策略：requests -> Playwright -> SeleniumBase
        ...
        # 策略1: 使用requests+BeautifulSoup（在线程池中执行同步方法）
        try:
            loop = asyncio.get_running_loop()
            return await loop.run_in_executor(None, self._extract_content_with_requests, url, max_chars)
        except Exception as e:
            errors.append(f"Requests: {str(e)}")
            logger.warning(f"Requests失败，尝试下一种方法: {str(e)}")
        
        # 策略2: 尝试Playwright（原生异步）
        if _check_playwright():
            try:
                return await self._extract_with_playwright(url, max_chars)
            except Exception as e:
                errors.append(f"Playwright: {str(e)}")
                logger.warning(f"Playwright失败，尝试下一种方法: {str(e)}")
        
        # 策略3: 尝试SeleniumBase（在线程池中执行同步方法）
        if _check_seleniumbase():
            try:
                loop = asyncio.get_running_loop()
                return await loop.run_in_executor(None, self._extract_with_seleniumbase, url, max_chars)
            except Exception as e:
                errors.append(f"SeleniumBase: {str(e)}")
                logger.warning(f"SeleniumBase失败: {str(e)}")
