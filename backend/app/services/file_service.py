...
try:
    ...
    logger.warning(f"高级文档处理库未安装: {e}")


class FileService:
    """文件处理服务"""

    # 图片上传配置 - 通过环境变量配置，避免硬编码
    ...
    async def upload_image_to_server(image_data: bytes, filename: str) -> Optional[str]:
        """上传图片到外部服务器（需配置 IMAGE_UPLOAD_URL 环境变量）"""
        upload_url = FileService.IMAGE_UPLOAD_URL
        if not upload_url:
            logger.debug("[图片上传] 未配置 IMAGE_UPLOAD_URL，跳过图片上传")
            return None
        try:
            form_data = aiohttp.FormData()
            form_data.add_field('file',
                              io.BytesIO(image_data),
                              filename=FileService._sanitize_filename(filename),
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.post(upload_url, data=form_data) as response:
                    if response.status == 200:
                        ...
                        logger.warning(f"图片上传失败，状态码: {response.status}")
                        return None
        except Exception as e:
            logger.warning(f"图片上传异常: {str(e)}")
            return None
            ...
            for page_num in range(doc.page_count):
                ...
                for img_index, img in enumerate(image_list):
                    try:
                        ...
                        logger.warning(f"提取PDF第{page_num+1}页图片{img_index+1}失败: {str(e)}")
                        continue
    ...
    def extract_images_from_docx(file_path: str) -> List[Tuple[bytes, str, int]]:
        ...
        try:
            ...
            for rel in rels.values():
                if "image" in rel.target_ref:
                    try:
                        ...
                        logger.warning(f"提取Word文档图片{img_index+1}失败: {str(e)}")
                        continue
            ...
            if doc:
                del doc
            gc.collect()
            logger.warning(f"Word文档图片提取失败: {str(e)}")
            return []
    ...
    async def save_uploaded_file(file: UploadFile) -> str:
        ...
        filename = FileService._sanitize_filename(file.filename or "unknown_file")
    ...
    async def process_uploaded_file(file: UploadFile) -> str:
        ...
        try:
            # 根据文件类型提取文本和图片（优先用扩展名判断）
            filename = file.filename or ""
            ext = filename.lower().split('.')[-1] if '.' in filename else ""
            
            if ext == "pdf" or file.content_type == "application/pdf":
                text = await FileService.extract_text_from_pdf(file_path)
            elif ext in ["docx", "doc"] or file.content_type == "application/vnd.openxmlformats-officedocument.wordprocessingml.document":
