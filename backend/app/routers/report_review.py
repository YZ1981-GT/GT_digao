...
async def get_result(session_id: str):
    ...
    return {
        "session_id": session_id,
        "findings": [f.model_dump() for f in confirmed],
...
@router.post("/export")
async def export_report(session_id: str = Form(...), format: str = Form("word")):
    """导出报告。"""
    findings = _findings.get(session_id, [])
    confirmed = [f for f in findings if f.confirmation_status == FindingConfirmationStatus.CONFIRMED]
    return {"status": "exported", "format": format, "findings_count": len(confirmed)}
